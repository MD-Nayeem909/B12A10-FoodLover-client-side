...
const ReviewDetails = () => {
	...
	const { _id, foodName, restaurantName, reviewText, location, rating, reviewerName, image, tags } = reviewData;
			...
			<section className="grid grid-cols-1 md:grid-cols-2 gap-8 my-12">
				<div>
					<div className="relative h-64 md:h-96 bg-base-300 shadow-md flex items-center justify-center overflow-hidden rounded-t-lg">
						<img src={image || '/placeholder.svg'} alt={foodName} className="h-full w-full object-cover" />
						<button
				...
				<div>
					<div className="px-6 md:px-0">
						<Link to={from} state={{ from: location, review: reviewData }} className="flex gap-2 font-semibold text-accent items-center">
							<ArrowLeft />
							Back to Reviews
						</Link>
						<h2 className="text-5xl font-bold my-4">{foodName}</h2>
						<div className="flex items-center gap-2 mb-4">
							<p className="text-xl font-semibold text-text-primary">{restaurantName}</p>
						...
						<div className="flex gap-3">
							{Array.isArray(tags) &&
								tags.map((tag) => (
									<button key={tag} className="rounded-full btn bg-[#9F62F230] btn-sm">
					...
					<div className="flex flex-col bg-base-100 p-6 mt-6 shadow-md rounded-lg">
						<span className="text-2xl font-semibold mb-6">Review Information</span>
						<div className="flex items-center gap-3">
							...
							<div>
								...
								<div className="text-sm opacity-50">crafts.by.sara@shop.net</div>
							</div>
						</div>
						<div className="flex flex-col text-accent mt-3 gap-2">
							...
							<div className="flex items-center gap-2">
								<div className="flex gap-1">
									...
									{Array.from({ length: 5 }).map((_, i) => (
										<span key={i} className={`text-lg ${i < Math.floor(rating) ? 'text-accent' : 'text-gray-300'}`}>
											★
										</span>
									))}
								</div>
							...
							<span>
								<strong>Status :</strong> <button className="btn btn-warning btn-xs rounded-full">On Sale</button>
							</span>
						</div>
					</div>
					<div className="w-full flex flex-col md:flex-row gap-4 mt-6">
						<Link to={`/edit-review/${_id}`} state={{ from: locationData, reviewData }} replace className="btn btn-primary">
							<Edit2 size={20} /> Edit Review
						</Link>
						<button onClick={() => handleRemoveReview(_id)} className="btn btn-error hover:text-white btn-outline">
